...
# -- Config file is relative path so that it works on any computer. This requires that node is run from certain directory. Change so that the config file is automatically found regardless of where node is run from
...
class UrRtdeDriver:
    ...
    def __init__(self):
        ...
        self.servoj_params      = [0.01,0.2,100]
    ...
    def connect_rtde(self):
        ...
        #print self.state_names, self.state_types	
        ...
        #self.con = rtde.RTDE(ROBOT_HOST, GRIPPER_PORT)
        self.con.connect()
    ...
    def parse_and_publish_state(self, state_object):
        ...
        #print("STATE:", state_object.actual_TCP_force)
        for i in range(len(state_object.actual_TCP_force)):
        	state_object.actual_TCP_force[i] = round(state_object.actual_TCP_force[i], 2)

        for i in range(len(state_object.actual_qd)):
        	state_object.actual_qd[i] = round(state_object.actual_qd[i], 2)

        for i in range(len(state_object.actual_q)):
        	state_object.actual_q[i] = round(state_object.actual_q[i], 2)

        for i in range(len(state_object.actual_current)):
        	state_object.actual_current[i] = round(state_object.actual_current[i], 2)

        self.forces = state_object.actual_TCP_force
        #print(state_object)
        ...
        joint_state_msg.effort = state_object.actual_current
