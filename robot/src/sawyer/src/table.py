...
class listener():
	def __init__(self):
		...
		# 	self.dir_path = self.home + '/ros_ws/teachbot_ws/src/teachbot_ros/Learner_Responses/module' + str(module_number) + '/'
	...
	def check_pickup(self, lift_position = z_table + box1_height - gripper_width):
		...
		self.startPose_container.position_z = lift_position + .2
	...
	def callback(self, data):
		...
		if (data.data == 0):
			...
			#Check picking box2 and placing in bin 1
			...
			self.startPose = self.limb.endpoint_pose()
			self.startPose_container = self.pose_to_cartclass(pose = self.startPose, cartclass = self.startPose_container)
			self.startPose_container.position_z = self.startPose_container.position_z + box2_height
			self.startPose_arg = self.cartclass_to_cartarg(cartclass = self.startPose_container, cartarg = self.startPose_arg)
			...
			#update current start pose
			self.startPose = self.limb.endpoint_pose()
			self.startPose_container = self.pose_to_cartclass(pose = self.startPose, cartclass = self.startPose_container)
			self.startPose_container.position_z = z_table + box2_height - gripper_width + .2
			self.startPose_arg = self.cartclass_to_cartarg(cartclass = self.startPose_container, cartarg = self.startPose_arg)

			self.newCartPose = self.pose_to_cartclass(pose = self.startPose, cartclass = self.newCartPose)
			self.newCartPose.position_z = z_table + box2_height - gripper_width
			self.newCartPose_arg = self.cartclass_to_cartarg(cartclass = self.newCartPose, cartarg = self.newCartPose_arg)
			...
			#some logic for detecting if the object was lifted
			if(self.compare_efforts(effort1 = self.effort[0], effort2 = self.effort[1])):
				#did not lift the shoe. Efforts are the same.
				grip_msg.data = 1
				self.gripper_pub.publish(grip_msg)
			...
			self.gripper_pub.publish(grip_msg)
			go_to.joint_angles(init_joint_arg)
			go_to.joint_angles(init_course_arg)

			rospy.sleep(2)
			go_to.joint_angles(init_course_bckwrds_arg)
			int_msg.data = 28
			self.iframe_sequence_pub.publish(int_msg)
			rospy.sleep(2)
			go_to.joint_angles(init_course_arg)
		 	...
		 	#Go to start of obstacle course
		 	go_to.joint_angles(init_course_arg)
		 	rospy.sleep(2)
			del self.waypoints[:]
			self.waypoint_limit = 7
			...
			self.waypoints.append(self.waypoint_init_arg)
		 	go_to.cartesian_pose(self.waypoint_init_arg)
		 	
			zeroG.constrained(zeroG_xyplane)
		 	self.eventnumber = 29

		elif(data.data == 29):
			#Check the points
			rospy.sleep(1)
			unique_msg.data = self.check_intersect(waypoint_list = self.waypoints, line_segment_list = first_obstacle_segments)
		 	if(unique_msg.data == []):
		 		...
		 		#good job. now can you find a path back?
		 		rospy.sleep(3)
		 		int_msg.data = 31
		 		self.iframe_sequence_pub.publish(int_msg)

		 	else:
		 		print(unique_msg.data)
		 		#[i,j] where i is the waypoint line segment and j is the obstacle line segment
		 		unique_msg.data = str(self.check_intersect(waypoint_list = self.waypoints, line_segment_list = first_obstacle_segments))
		 		self.unique_input_pub.publish(unique_msg)

		elif(data.data == 30):
			#Wrong waypoints were sent.
			for x in range(0, int(unique_msg.data[1])+1):
			...
			#correct answer
			#Move to the end of the obstacle course and give constrained control.
			go_to.joint_angles(init_course_bckwrds_arg)
			rospy.sleep(2)
			zeroG.constrained(zeroG_ori_constraints)
...
if __name__ == '__main__':
	...
	success_init_joint_arg = go_to.joint_angle_arg(joint_angles = [-0.383508789063,-0.348434570313,-1.62057226562,1.12419726563,-1.88032324219,-1.72695605469,3.28075195312])
	...
	success_pickup_cart_arg = go_to.cartesian_pose_arg(joint_angles = [-0.305973632813,0.156454101563,-1.3836171875,1.46058203125,-1.43931152344,-1.3310546875,3.05668261719])
	success_above_pickup_cart_arg = go_to.cartesian_pose_arg(joint_angles =[-0.383508789063,-0.348434570313,-1.62057226562,1.12419726563,-1.88032324219,-1.72695605469,3.28075195312])
