...
class listener(Module):
	...
	def rx_command(self, data):
		...
		if seq.idn==0:
			...
			self.limb.adjustOrientationTo(x=0.707, y=0.707)
			if not self.devMode:
				upPose = self.limb.endpoint_pose()
				self.limb.adjustPoseTo('position','z',Module.Z_TABLE+Module.BOX_HEIGHT/2)
				
				#Complete grip
				self.close_gripper()

				#Show off the proper gripping
				self.limb.go_to_cartesian_pose(endpoint_pose = upPose)
				self.limb.go_to_joint_angles(init_joint_arg)
			...
			self.limb.go_to_cartesian_pose(joint_angles = above_bin_cart_arg)

			self.startPose = self.limb.endpoint_pose()
			self.newCartPose = self.pose_to_cartclass(pose = self.startPose, cartclass = self.newCartPose)
			
			self.newCartPose.position_z = Module.Z_TABLE + bin_height + box2_height - gripper_width
			...
			self.limb.go_to_joint_angles(self.newJointAngles_arg)
			int_msg.data = 7
			self.iframe_sequence_pub.publish(int_msg)
			...
			self.newCartPose.position_z = Module.Z_TABLE + bin_depth + box2_height - gripper_width
			...
			self.limb.go_to_joint_angles(success_init_joint_arg)
			...
			self.limb.go_to_cartesian_pose(joint_angles = into_second_bin_cart_arg)
			self.limb.go_to_cartesian_pose(joint_angles = self.startPose_arg)
			...
			self.newCartPose.position_z = Module.Z_TABLE + box2_height - gripper_width + bin_height + .2
			...
			self.limb.go_to_cartesian_pose(joint_angles = above_second_bin_cart_arg)
			int_msg.data = 14
			self.iframe_sequence_pub.publish(int_msg)
			...
			self.startPose = self.limb.endpoint_pose()
			self.newCartPose = self.pose_to_cartclass(pose = self.startPose, cartclass = self.startPose_container)
			self.newCartPose.position_z = Module.Z_TABLE + box2_height + bin_depth - gripper_width
			self.newCartPose_arg = self.cartclass_to_cartarg(cartclass = self.newCartPose, cartarg = self.newCartPose_arg)
			self.limb.go_to_cartesian_pose(joint_angles = self.newCartPose_arg)
			...
			self.startPose = self.limb.endpoint_pose()
			self.startPose_container = self.pose_to_cartclass(pose = self.startPose, cartclass = self.startPose_container)
			self.startPose_container.position_z = Module.Z_TABLE + box2_height - gripper_width + 0.01
			self.startPose_arg = self.cartclass_to_cartarg(cartclass = self.startPose_container, cartarg = self.startPose_arg)
			self.limb.go_to_cartesian_pose(joint_angles = self.startPose_arg)
			self.close_gripper()
			...
			self.limb.go_to_cartesian_pose(joint_angles = self.startPose_arg)
			self.limb.go_to_cartesian_pose(joint_angles = above_third_box_cart_arg)

			self.startPose = self.limb.endpoint_pose()
			self.startPose_container = self.pose_to_cartclass(pose = self.startPose, cartclass = self.startPose_container)
			self.startPose_container.position_z = Module.Z_TABLE + box2_height - gripper_width
			...
			if(self.compare_efforts(effort1 = self.effort[0], effort2 = self.effort[1])):
				...
				self.open_gripper()
				self.limb.go_to_cartesian_pose(joint_angles = self.startPose_arg)
				self.limb.go_to_joint_angles(empty_angle_arg)
			...
			self.limb.go_to_joint_angles(self.savedLocations[0])
			int_msg.data = 34
			self.iframe_sequence_pub.publish(int_msg)
			...
			self.limb.go_to_joint_angles(self.savedLocations[1])
			int_msg.data = 35
			self.iframe_sequence_pub.publish(int_msg)
			...
			self.limb.go_to_joint_angles(self.savedLocations[2])
			int_msg.data = 36
			self.iframe_sequence_pub.publish(int_msg)
			...
			self.limb.go_to_joint_angles(self.savedLocations[3])
			int_msg.data = 37
			self.iframe_sequence_pub.publish(int_msg)
			...
			self.limb.go_to_joint_angles(self.savedLocations[4])
			int_msg.data = 38
			self.iframe_sequence_pub.publish(int_msg)
